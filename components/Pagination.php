...
class Pagination
{
    ...
    public function get()
    {
        ...
        $html = '<nav class="d-flex justify-content-center"><ul class="pagination pg-blue" style="margin: 0">';

        # Генерируем ссылки
        for ($page = $limits[0]; $page <= $limits[1]; $page++) {
            # Если текущая это текущая страница, ссылки нет и добавляется класс active
            if ($page == $this->current_page) {
                $links .= '<li class="page-item active"><a class="page-link disabled">'. $page .'<span class="sr-only">(current)</span></a></li>';
            } else {
